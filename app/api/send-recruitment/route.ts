...
export async function POST(req: NextRequest) {
  if (!toEmail || !fromEmail) {
    console.error(
      "Thiếu TO_EMAIL_ADDRESS hoặc FROM_EMAIL_ADDRESS trong .env.local"
    );
    return NextResponse.json(
      { error: "Lỗi cấu hình máy chủ không thể gửi email." },
      { status: 500 }
    );
  }
  if (!resendApiKey) {
    return NextResponse.json(
      { error: "Lỗi cấu hình dịch vụ email máy chủ." },
      { status: 500 }
    );
  }
  ...
  try {
      ...
      return NextResponse.json(
        {
          ...
          details: errorMessages || "Vui lòng kiểm tra lại thông tin.",
        },
        { status: 400 }
      );
    }
    ...
    if (!cvFile || cvFile.size === 0) {
      return NextResponse.json(
        { error: "CV là bắt buộc và không được để trống." },
        { status: 400 }
      );
    }
    ...
    if (cvFile.size > 5 * 1024 * 1024) {
      return NextResponse.json(
        { error: "Kích thước file CV không được vượt quá 5MB." },
        { status: 400 }
      );
    }
    // Kiểm tra loại file
    ...
    if (!allowedTypes.includes(cvFile.type)) {
      return NextResponse.json(
        {
          error:
            "Định dạng file CV không hợp lệ. Chỉ chấp nhận .pdf, .doc, .docx.",
        },
        { status: 400 }
      );
    }
    ...
    if (error) {
      ...
      return NextResponse.json(
        {
          error: "Không thể gửi email ứng tuyển. Vui lòng thử lại sau.",
    ...
    return NextResponse.json(
      { message: "Hồ sơ của bạn đã được gửi thành công!", emailId: data?.id },
      { status: 200 }
    );
  } catch (e: any) {
    ...
    return NextResponse.json(
      {
        error: "Đã xảy ra lỗi không mong muốn từ máy chủ.",
        details: e.message,
      },
      { status: 500 }
    );
